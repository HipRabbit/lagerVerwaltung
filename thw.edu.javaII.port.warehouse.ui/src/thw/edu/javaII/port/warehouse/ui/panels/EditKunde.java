...
public class EditKunde extends JDialog {
    ...
    private void initializeUI() {
        ...
        ((AbstractDocument) txtVorname.getDocument()).setDocumentFilter(letterOnlyFilter);
        add(txtVorname, "growx");

        add(new JLabel("Nachname:"));
        txtNachname = new JTextField(kunde != null ? kunde.getNachname() : "", 20);
        ((AbstractDocument) txtNachname.getDocument()).setDocumentFilter(letterOnlyFilter);
        add(txtNachname, "growx");

        add(new JLabel("Lieferadresse:"));
        txtLieferadresse = new JTextField(kunde != null ? kunde.getLieferadresse() : "", 20);
        ((AbstractDocument) txtLieferadresse.getDocument()).setDocumentFilter(letterOnlyFilter);
        ...
        ((AbstractDocument) txtRechnungsadresse.getDocument()).setDocumentFilter(letterOnlyFilter);
    ...
    private class LetterOnlyFilter extends DocumentFilter {
        ...
        private void handleInvalidInput() {
            ...
            if (!hasShownWarning) {
                JOptionPane.showMessageDialog(EditKunde.this, 
