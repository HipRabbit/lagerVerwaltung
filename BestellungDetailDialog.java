        ...
        try {
            if (hasChanges) {
                ...
                if (success) {
                    ...
                    // Aktualisiere die BestellungPage, falls sie angezeigt wird
                    if (getOwner() instanceof LagerUI) {
                        LagerUI lagerUI = (LagerUI) getOwner();
                        Component currentPanel = lagerUI.getContentPane().getComponent(0); // contentPane enthält ein Panel
                        if (currentPanel instanceof BestellungPage) {
                            ((BestellungPage) currentPanel).updateTable();
                        }
                        // Kein Fehler, wenn BestellungPage nicht angezeigt wird, da sie beim Öffnen neu geladen wird
                    }
                    dispose();
                } else {
